...
// The format string will prepend the value to argv.
func AssertNil(value any, message string, argv ...any) {
	assert(value == nil, message, append([]any{value}, argv...)...)
}
...
// The format string will prepend the values to argv.
...
// The format string wil prepend the value and the casted value, respectively, to argv
func AssertType[T any](value any, message string, argv ...any) T {
	v, ok := value.(T)
	assert(ok, message, append([]any{value, v}, argv...)...)
	return v
}
...
func AssertTypeNotZero[T comparable](value any, message string, argv ...any) T {
	var v T
	var viz interface{ IsZero() bool }
	var ok bool
	v, ok = value.(T)
	assert(ok, message, append([]any{value, v}, argv...)...)
	viz, ok = value.(interface{ IsZero() bool })
	if ok {
		assert(!viz.IsZero(), message, append([]any{value, v}, argv...)...)
	}
	var zero T
	assert(v != zero, message, append([]any{value, zero}, argv...)...)
	return v
}
