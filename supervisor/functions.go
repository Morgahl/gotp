...
func ContextStartChild[S process.Sendable](pctx *process.Context, supervisor S, child Supervisable) gotp.Term {
	if resp, ok := gen_server.ContextCall[ChildSpec, gotp.Term](supervisor, pctx, child.ChildSpec(), 0); ok {
		switch resp := resp.(type) {
		case AlreadyStarted, process.PID, error:
			return resp
		default:
...
func ContextStopChild[S process.Sendable](pctx *process.Context, supervisor S, pid process.PID) gotp.Term {
	if resp, ok := gen_server.ContextCall[process.PID, gotp.Term](supervisor, pctx, pid, 0); ok {
		switch resp := resp.(type) {
		case bool:
		default:
