...
var _ gen_server.GenServer[gotp.Options, gotp.Term, gotp.Term, gotp.Term, gotp.Term] = &server{}
...
func (s *server) Init(pctx *process.Context, opts gotp.Options) (cont gen_server.Continue[gotp.Term], err error) {
	pctx.TrapExit(true)
...
func (s *server) HandleCall(pctx *process.Context, msg gotp.Term, _ process.PID) (resp gen_server.Response[gotp.Term], cont gen_server.Continue[gotp.Term], err error) {
	switch m := msg.(type) {
	// starting child
...
func (s *server) HandleCast(pctx *process.Context, msg gotp.Term) (cont gen_server.Continue[gotp.Term], err error) {
	return gen_server.NoCont[gotp.Term](), nil
}

func (s *server) HandleContinue(pctx *process.Context, arg gotp.Term) (cont gen_server.Continue[gotp.Term], err error) {
	return gen_server.NoCont[gotp.Term](), nil
}

func (s *server) HandleInfo(pctx *process.Context, info gotp.Term) (cont gen_server.Continue[gotp.Term], err error) {
	defer func() {
		if r := recover(); r != nil {
		}
	}()
